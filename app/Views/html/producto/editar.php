                ...
                <form id="form_global" data-fv-icon-validating="bi bi-arrow-repeat bi-spin">
                    ...
                    <div class="mb-3">
                        <div class="form-group position-relative has-feedback">
                            ...
                            <div class="file-loading">
                                <input type="file" id="imagen" name="imagen" class="form-control d-none" required="false"
                                    accept="image/*" data-fv-field="imagen">
                            </div>
                            <div class="messages_error_eva" id="messages_error_imagen"></div>
                            <div id="preview-imagen-imagen" class="position-relative preview-imagen mt-2">
                                <img src="<?= base_url($producto["imagen"]) ?>" alt="Imagen recortada" class="rounded">
                    ...
                    <div class="mb-3 p-none">
                        ...
                        <div class="form-group position-relative">
                            <textarea required class="form-control" id="descripcion" name="descripcion"><?= $producto["descripcion"] ?></textarea>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    let editarAtributo = false;
    let editarImagen = false;
    $(function () {
        var opciones = {
            ...
            ajaxEnviar: function (data) {
                ...
                formDataGlobal.append('editarAtributo', editarAtributo.toString());
                formDataGlobal.append('editarImagen', editarImagen.toString());
        ...
        var newOption = new Option("<?= $producto["categoria_nombre"] ?>", "<?= $producto["idcategoria"] ?>", false, false);
        ...
        var dataAtributeComplete = <?= json_encode($atributoProducto) ?>;

        // Mapear los datos a un formato compatible con Select2
        const dataAtributo = Object.values(
            dataAtributeComplete.reduce((acc, attr) => {
                acc[attr.idatributo] = {
                    id: attr.idatributo,
                    text: attr.atributo_nombre
                };
                return acc;
            }, {})
        );

        // Crear y añadir las nuevas opciones
        dataAtributo.forEach(function(item) {
            var newOption = new Option(item.text, item.id, false, false);
            $("#idatributo").append(newOption);
        });

        // Seleccionar los valores
        $("#idatributo").val(dataAtributo.map(e => e.id)).trigger('change');
                ...
                list: function(){
                   try {
                        if(document.getElementById(accordeonAtributo.id)){
                            ...
                            this.atributos[ultimoIndice]["data"]["isSelect2"] = false;
                ...
                createSelect2: function(atributo){
                    try {
                        ...
                        if($(idSelectTemp) && !$(idSelectTemp).data('select2')){
                            ...
                            if(this.atributos[i].data.nombre.length > 0){
                                // Crear y añadir las nuevas opciones
                                this.atributos[i].data.nombre.forEach(function(item) {
                                    var newOption = new Option(item.text, item.id, false, false);
                                    $(idSelectTemp).append(newOption);
                                });

                                // Seleccionar los valores
                                $(idSelectTemp).val(this.atributos[i].data.nombre.map(e => e.id)).trigger('change');
                            }
                        }
                        return true;
                    } catch (error) {
        ...
        dataAtributo.forEach(function(item) {
            const atributo = {
                ...
                isNew: false,
                data: {
                    nombre: dataAtributeComplete.filter(e => {
                        if(e.idatributo == item.id){
                            return e;
                        }
                    }).map(e => {
                        return {
                            "text": e.valoratributo_nombre,
                            "id": e.idvaloratributo,
                            "isNew": false,
                        }
                    }) ,
                    isSelect2: false

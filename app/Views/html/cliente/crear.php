  ...
  $(function(){
    var opciones = {
        ...
        ajaxEnviar: function (data) {
            return $.ajax({
                url: '<?= base_url(route_to('cliente.actions')) ?>',
                type: 'POST',
                data: data,
                dataType: 'JSON',
                beforeSend: function () {
                ...
                ci: {
                    validators: {
                        ...
                        remote: {
                            ...
                            url: '<?= base_url(route_to('cliente.validar.ci')) ?>',
                            data: function (validator, $field, value) {
                                return {
                                    "<?= csrf_token() ?>": "<?= csrf_hash() ?>",
                                    ci: validator.getFieldElements('ci').val(),
        ...
        opciones.ajaxEnviar($form.serialize())
                .done(function (data) {
                    if (data.resp) {
                        ...
                        setTimeout(function() {
                            window.location = '<?= base_url(route_to('cliente')) ?>';
                        }, 2500);
                    } else {
                        $('#form_global').data('formValidation').resetForm();
                        $('#form_global').find('input, textarea, button, select').prop('disabled', false);

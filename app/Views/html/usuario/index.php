            ...
            <div class="col-xl-12">
                <div class="d-flex justify-content-between align-items-sm-end">
                    ...
                    <ul class="nav nav-tabs border-bottom-0 d-none d-sm-flex">
                        <li class="nav-item">
                            <a class="nav-link <?= isset($_GET['papelera']) ? '' : 'active' ?>" aria-current="page" href="<?= base_url(route_to('usuario')) ?>">
                                Registros activos (<?= $registros_no_eliminados ?>)
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= !isset($_GET['papelera']) ? '' : 'active' ?>" href="<?= base_url(route_to('usuario')) ?>?papelera=1">
...
<!-- Modal -->
<div id="resetpass" class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <form id="form_resetpass">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Recupera la contraseña</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="pwd" class="form-label">Cambiar contraseña:</label>
                        <input required type="password" data-eye class="form-control" id="pass" placeholder="Ingrese su nueva contraseña">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="submit" id="save_resetpass" class="btn btn-success">Guardar</button>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="permisosModalLabel">
                    <i class="bi bi-person-badge"></i> Asignación de Permisos de Usuario
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-1"><strong>Usuario:</strong> <span id="userName">Juan Pérez</span></p>
                                <p class="mb-0"><strong>Email:</strong> <span id="userEmail">juan.perez@empresa.com</span></p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-1"><strong>Rol:</strong> <span id="userRole">Editor</span></p>
                                <p class="mb-0"><strong>Estado:</strong> <span id="modal_estado_usuario"></span></p>
                            </div>
                            <div class="col-md-12">
...
<script type="text/javascript">
    var datatable;
    $(function() {
        var column = ['Nombre completo', 'Usuario', 'Correo electrónico', 'Rol de usuario', 'Estado de Acceso', 'Contraseña'];
        ...
        datatable = $('#listartable').DataTable({
            ...
            serverSide: true,
            "ajax": {
                url: '<?php echo base_url(route_to('usuario.actions')); ?>',
                "type": "POST",
                "data": function(d) {
            ...
            columns: [{
                ...
                }, 
                {
                    ...
                    "render": function(d, t, f) {
                        return `<a class="btn btn-warning btn-sm" data-action="pass" data-id="${f.idusuario}" href="#">Cambiar <i class="fas fa-lock" aria-hidden="true"></i></a>`;
                    },
                    sDefaultContent: "",
                    className: 'gradeA class-tipo',
                    "orderable": false
        ...
        $('#admin_cr').on('click', 'a[data-action="estado"]', function(e) {
            ...
            $.ajax({
                url: '<?= base_url(route_to('usuario.actions')); ?>',
                type: 'POST',
                data: {
                    'action': 'estado',
                    'id': id,
        ...
        $('#admin_cr').on('click', 'a[data-action="pass"]', function(e) {
            e.preventDefault();
            var id = $(this).data('id');
            $('#idlogin').val(id)
        ...
        $("input[type='password'][data-eye]").each(function(i) {
            const $this = $(this);
            const wrapperId = "eye-password-" + i;

            // Envolver input
            $this.wrap(`<div id="${wrapperId}" style="position:relative"></div>`);

            // Botón toggle
            const $toggle = $(`<button type="button" class="btn btn-sm btn-primary pass" 
            ...
            $toggle.on("click", function() {
                const isShown = $this.attr("type") === "text";
                $this.attr("type", isShown ? "password" : "text");
                $(this)
                    .text(isShown ? "Ver" : "Ocultar")
                    .toggleClass("btn-outline-primary", !isShown)
                    .toggleClass("btn-primary", isShown);
            });
        });
        ...
        $('#admin_cr').on('click', 'a[data-action="edit-usuario"]', function() {
        ...
        $('#admin_cr').on('click', 'a[data-action="asignacion-perfil"]', function() {
            ...
            let dataEstructure = data.split(',');
            let idusuario = dataEstructure[0].split(':')[1];
            let tipo = dataEstructure[2].split(':')[1];
            let datosUsuario = obtenerDatosPorId(idusuario);
            // console.log("datosUsuario", datosUsuario)
            // Actualizar información del usuario en el modal
            $('#userName').text(datosUsuario.nombre || 'N/A');
            $('#userEmail').text(datosUsuario.correo_elec || 'N/A');
            $('#userRole').text(datosUsuario.tipo_text || 'N/A');
            $('#modal_estado_usuario').html(datosUsuario.estado_usuario_text || 'N/A');

            $('#permisosModal').modal('show');
            $('#reset_permisos').val(idusuario);

            cargarPermisos(idusuario).then(response => {
                ...
                if(dataLocal.length == 0){
                    $('#permisosModal tbody').html('<tr><td colspan="5" class="text-center text-muted">No hay permisos disponibles para este usuario.</td></tr>');
                    return;
                }
        ...
        function rowTemplatePermisos(permiso){
            const {opcion_menu = '', leer = 0, crear = 0, modificar = 0, eliminar = 0} = permiso;
            try {
                return `<tr id-permiso="${permiso.idpermiso}">
                            ...
                            <td class="text-center">
                                <div class="form-check d-inline-block">
                                    ${rowTemplateCheck(modificar, 'modificar')}
                                </div>
                            </td>
                            <td class="text-center">
                                <div class="form-check d-inline-block">
        ...
        async function cargarPermisos(idusuario) {
            return $.ajax({
                url: '<?php echo base_url(route_to('usuario.actions')); ?>',
                type: 'POST',
                data: {
        ...
        $('#permisosModal tbody').on('change', '.form-check-input', function() {
            ...
            const permisoId = row.attr('id-permiso');

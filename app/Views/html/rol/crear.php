    ...
    $(function() {
            ...
            const API = {
                enviar: function(data) {
                    return $.ajax({
                        url: '<?= base_url(route_to('rol.actions')) ?>',
                        type: 'POST',
                        data: data,
                        dataType: 'JSON',
                        beforeSend: function() {
                            ...
                            nombre: {
                                validators: {
                                    ...
                                    remote: {
                                        ...
                                        url: '<?= base_url(route_to('rol.validar.nombre')) ?>',
                                        type: 'GET',
                ...
                API.enviar($form.serialize())
                    .done(function(data) {
                        if (data.resp) {
                            ...
                            setTimeout(() => window.location = '<?= base_url(route_to('rol')) ?>', 2500);
                        } else {
            ...
            $('#cancelar').on('click', function() {
                window.location = '<?= base_url(route_to('rol')) ?>';
            });
        });
</script>
